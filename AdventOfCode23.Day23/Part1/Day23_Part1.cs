...
public static class Day23_Part1
{
    static int height;
    static int width;
    static char[,] grid;
    ...
    static void Walk()
    {
        Walker initialWalker = new Walker(start, down, 0, new());
    ...
    {
        return grid[walker.Position.Y + walker.Direction.Y, walker.Position.X + walker.Direction.X] != '#';
    }
    ...
    static List<Point2d> GetSpawnDirections(Point2d walkerPosition, Point2d walkerDirection) 
    {
        ...
        if (grid[walkerPosition.Y - 1, walkerPosition.X] == 'v')
            directions.Add(down);
    ...
    static bool IsNode(this Point2d position)
    {
        ...
        if (directionChars.Contains(grid[position.Y + 1, position.X]))
            count++;
        if (directionChars.Contains(grid[position.Y - 1, position.X]))
            count++;
        if (directionChars.Contains(grid[position.Y, position.X + 1]))
            count++;
        if (directionChars.Contains(grid[position.Y, position.X - 1]))
            count++;
